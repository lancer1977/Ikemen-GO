...
--cell data storage
start.c = {}
for i = 1, --[[gameOption('Config.Players')]]20 do
...
function start.f_getSelectRecordLayout(side, index)
	local offset = motif.select_info['p' .. side .. '_name_offset']
	local spacing = motif.select_info['p' .. side .. '_name_spacing']
	local inward = side == 1 and 80 or -80
	return offset[1] + (index - 1) * spacing[1] + inward, offset[2] + (index - 1) * spacing[2]
end

function start.f_getSelectRecordIconCenter(sel, side, index)
	...
	return start.f_getSelectRecordLayout(side, index)
end

function start.f_getSelectRecordOffsetY()
	return 36
end
...
function start.f_getCharRecordText(ref)
	if ref == nil or not start.f_getCharData(ref) then
		return {}
	end
...
function start.f_updateCharRecords(winnerSide)
	...
	local seen = {}
	for _, sel in ipairs(start.p[winnerSide].t_selected or {}) do
		if sel.ref and not seen[sel.ref] then
			start.f_updateCharRecord(sel.ref, true, 1)
			seen[sel.ref] = true
		end
	end
	seen = {}
	for _, sel in ipairs(start.p[loserSide].t_selected or {}) do
		if sel.ref and not seen[sel.ref] then
			start.f_updateCharRecord(sel.ref, false, 1)
			seen[sel.ref] = true
		end
	end
end

function start.f_updateRoundCharRecords(winnerSide)
	if start.roundRecordSaved or (winnerSide ~= 1 and winnerSide ~= 2) then
		return
	end
	local loserSide = winnerSide == 1 and 2 or 1
	local seen = {}
	for i = 1, teamsize() * 2 do
		if player(i) then
			local side = teamside()
			local member = memberno()
			local ref = nil
			if start.p[side] ~= nil and start.p[side].t_selected ~= nil then
				local sel = start.p[side].t_selected[member + 1]
				if sel ~= nil then
					ref = sel.ref
				end
			end
			if ref ~= nil and not seen[side .. ':' .. member] then
				if side == winnerSide and win() and alive() then
					start.f_updateCharRecord(ref, true, 1)
					seen[side .. ':' .. member] = true
				elseif side == loserSide and not alive() and life() <= 0 then
					start.f_updateCharRecord(ref, false, 1)
					seen[side .. ':' .. member] = true
				end
			end
		end
	end
	start.roundRecordSaved = true
	f_saveStats()
end
...
function start.f_getTeamMemberRef(side, member)
...
function start.f_getActiveCharRef(side)
	...
	if start.p[side].teamMode == 2 then --Turns
		for i = 1, teamsize() * 2 do
			if player(i) and teamside() == side then
				local ref = start.f_getTeamMemberRef(side, memberno())
				if ref ~= nil then
					return ref
				end
			end
	...
	return start.p[side].t_selected[1].ref
end
...
-- turns mode life recovery or solo-vs-team round recovery
function start.f_turnsRecovery()
	...
	for side = 1, 2 do
		if start.p[side] and start.p[side].teamMode ~= 0 then
			teamRound = true
			break
		end
	end
	if not teamRound then
		return
	end
	for i = 1, teamsize() * 2 do
		if player(i) then
...
function launchFight(data)
	...
	while true do
		...
		local winner
		winner, t_gameStats = start.f_game(t.lua)
		start.f_recordCharMatchResult(winner)
